...
GROWTH_TUTOR_PROVIDER which provider backs the tutor role: none (the default), replay or
                      ...
                      and a key without this variable wires nothing, because 07's budget guard,
                      usage accounting and audit trail are not built yet. With no tutor
...
def build_tutor(env):
   ...
   trail at the provider seam, and P1 has built none of the three, so a billed role is never wired
   by the accident of a key sitting in the environment. Without GROWTH_TUTOR_PROVIDER there is no
   tutor, and app/feedback/tutor.py's no-provider degradation returns the deterministic payload
   with no sentence.
   """
