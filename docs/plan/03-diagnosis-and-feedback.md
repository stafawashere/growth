...
Each judged point is sampled three times: two samples at temperature 0 with the standard prompt, and one sample with a strictness-varied prompt that states the stricter reading of `does_not_earn`. Agreement across all three publishes the decision. Any disagreement sets `decision` to `uncertain`, writes the point to `review_queue`, and shows it to the student as provisional. **Corrected 2026-09-24 (P3):** the routed grader, Sonnet 5, rejects a non-default temperature (a 400 through the API, and the Claude Code CLI has no temperature option), so the two standard samples run at the model's own setting; they remain two independent samples of the same prompt, which is what the disagreement detector needs. The standard prompt is the liberal one (`prompts/grader/point_liberal_v1.md`) and the strictness-varied one is `point_strict_v1.md`.
