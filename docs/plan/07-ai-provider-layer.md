...
Every temperature marked `[inferred]` is a tunable in `12-open-questions.md`. The three zeros are not tunables: a grader, a verifier and a transcriber that vary run to run make the agreement check meaningless, because the disagreement it detects would be sampling noise rather than genuine uncertainty. Track 3 is explicit that majority voting should be treated as a disagreement detector rather than an accuracy booster (https://arxiv.org/pdf/2203.11171 for the underlying self-consistency result [verified]; the reframing is [inferred] in track 3), and a disagreement detector only works when the samples were supposed to agree.
...
| generator | Anthropic claude-opus-5 | Gemini 3.8 Flash (batch) | none; the bank is pre-generated |
| verifier | Anthropic claude-opus-5 | Gemini 3.8 Flash (batch) | none; verification waits |
| grader | Anthropic claude-sonnet-5 | Gemini 3.8 Flash | none; grading is queued |
| diagnostician | Anthropic claude-opus-5 | Gemini 3.8 Flash | Ollama DeepSeek-R1-Distill-Qwen-14B, feedback marked provisional |
| transcriber | Anthropic claude-haiku-4-5 | Gemini 3.8 Flash | none; the student types the answer instead |
...
| Data policy | unknown from the loaded pages | | |
...
Known traps. The implicit caching minimum of 4,096 tokens on the 3.x Flash line is four times Anthropic's Sonnet 5 minimum and eight times the Opus 5 minimum, so a prefix sized for Anthropic will not cache on Gemini. Since Gemini is the secondary for every role, a fallback silently loses the cache discount unless the prefix clears 4,096 tokens, and the adapter records that as a distinct condition rather than as an ordinary cache miss. Explicit caches are unavailable on the Interactions path, so if this project ever adopts Interactions for server-side state it loses explicit cache control; the adapter therefore uses `generateContent`. Gemini's data policy is unknown from the loaded pages, which is itself a reason it is a secondary rather than a default, and it is an open question in `12-open-questions.md`.
...
```json
{
  ...
  "diagnostician": {
    "deployments": [
      {"order": 1, "provider": "anthropic", "model": "claude-opus-5",  "allowed_fails": 2, "cooldown_s": 60},
      {"order": 2, "provider": "gemini",    "model": "gemini-3.8-flash", "allowed_fails": 2, "cooldown_s": 60},
...
One Anthropic batch detail the job builder must respect: `max_tokens: 0` is rejected inside a batch, and an oversized batch returns 413 `request_too_large` against a limit of 100,000 requests or 256 MB (same URL [verified]). The generator's batches are chunked below both bounds.
