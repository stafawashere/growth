...
**Corrected 2026-09-20: this eval moves onto the template gate and stops being a paid model eval.** Under the template architecture in `13-ai-engineering.md`, generation authors one parameterised template per archetype and the backend instantiates it, so replaying a frozen parameter draw is deterministic code that produces the identical item every time. `tools/template_trial.py` `run_draws` already asserts key equivalence at `KEY_DISAGREEMENT_BAR = 0`, the declared representation with a figure spec where the representation is graphical, every distractor's `error_path` resolving to an active BC-ERR id and every `point_type_id` resolving to a BC-PT id, over 300 seeded draws rather than over one frozen draw. Two of the four assertions above were already re-expressed structurally by 13, because asserting exact model output is a test of the model rather than of the template. What remains is the calculator boundary, which is added to the gate rather than kept as a separate eval. The consequence is that golden set 1 costs $0.00 in model calls, runs on every template rather than on one item per family, and has 300 times the sample. The one thing that still costs money is re-authoring templates after a prompt version bump, at $7.51 for all 139, and that is a generation event rather than an eval. See `14-token-economy.md`.
...
**Corrected 2026-09-20, the sampling and the cadence.** `13-ai-engineering.md` computed that Cohen's kappa at 5 responses on 30 point types gives a 95 percent interval 0.2652 wide, which cannot separate the cited 0.47 from the cited 0.56, and replaced per-point-type kappa on the P3 gate with per-point-type exact match reported with its count. Exact match against fixed operator labels is settled by one grader sample; the escalation rate is the only metric here that needs three, and it is computed free and continuously on real gradings rather than on this set. So the monthly canary, whose job is to catch a provider-side model update behind an unchanged id, runs one sample, and three samples run only on a full run, meaning a grader template bump or a model id change. That is $2.16 a canary run against $6.48, and $32.40 over the cycle against $58.32. What is given up is a monthly escalation rate on the adversarial set specifically, and the check on that trade is whether the canary and the full run ever disagree in direction on exact match over two full runs. See `14-token-economy.md`.

**Corrected 2026-09-20, which points reach the grader at all.** `03-diagnosis-and-feedback.md` states that a point whose BC-PT record answers no to `justification_required`, `interpretation_required` and `hypotheses_required`, and whose `earns` text is fully expressible as one of four deterministic checks, is decided by the checker and never reaches a model. 17 of the 76 active BC-PT records answer yes to at least one of those three fields [measured 2026-09-20 over `data/scoring_points.json`: 76 active records, 59 answer no to all three, 14 require justification, 3 require interpretation, 3 require hypotheses]. Golden set 2 therefore targets the point types that actually reach a model, and the 30 point types it is built on are drawn from those rather than from the full 76. The second condition, that the `earns` text be expressible as one of the four checks, is a per-record judgement the data does not carry, and labelling all 76 against it is a P3 entry task that costs nothing and needs no key.
...
**P3, FRQ capture and per-point grading.** All 76 active BC-PT records are labelled for whether the `earns` text is fully expressible as one of the four deterministic checks, which is what decides how many judged points reach a model at all; this is an offline labelling pass that costs nothing and needs no key, and it is an entry task rather than an exit one. Golden set 2 exists at 5 responses on at least 30 BC-PT ids drawn from the point types that reach a model, with aggregate kappa reported with its interval, per-point-type exact match reported with its count, and the escalation threshold set. Golden set 3 exists and the read-back gate is tuned against its low-light and blur subsets. The share of grading errors attributable to transcription is measured on golden set 3 and compared against the 87 percent figure from the AIED study [single-source, https://arxiv.org/abs/2605.19043]. Leniency calibration against the published per-question means is run and its direction and magnitude reported. Free-response attempts per week hold above a non-zero floor for the duration of the P3 trial, and the read-back abandonment rate is measured and published (R11).

**P4, generation for all 139 archetypes.** Golden set 1 exists for every archetype family shipped in Units 1 to 3, which is where it enters (R31), and as corrected on 2026-09-20 it is satisfied by the template gate passing on those families rather than by a separate paid eval run, at 300 seeded draws per template and with the calculator boundary added to the gate's eleven checks. Key error rate on a fresh 100-item audited sample is at or below the threshold set in P1. The duplicate gate is validated on a labelled sample rather than trusted at its declared thresholds of MinHash Jaccard 0.8 over 5-grams and embedding cosine 0.85 [single-source]. Monte Carlo family checks over a few hundred parameter draws pass for every archetype. No archetype lacking `point_types` serves an FRQ-shaped item.
