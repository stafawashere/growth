...
- Whether AP Calculus BC receives a Bluebook reference sheet. The Bluebook tools page says a reference sheet with commonly used formulas appears on all tests with math questions (https://bluebook.collegeboard.org/students/tools [verified, the sentence]); no Calculus-specific College Board source confirms or denies it, and the CED and scoring guidelines do not mention one. Affects [04-item-generation.md](04-item-generation.md) (what an item may assume the student can look up) and [05-assessment-modes.md](05-assessment-modes.md). The plan builds the bank as if no sheet exists. Settled by: a College Board page or a 2027 sample booklet naming the Calculus reference sheet, or a screenshot from a Bluebook practice test for AP Calculus. Restated 2026-09-24 at P5: still open. The mock, the part drills and the unit check show no reference sheet and say so on the setup screen (`content/assessment/form_2027.json` `reference_sheet`), the conservative direction, because a student who has practised without one is over-prepared if one appears. What would settle it is unchanged, and the practice app is the fastest route: opening a Bluebook practice test for AP Calculus BC shows whether a reference sheet control is there. If one is, the change is a data edit to the form template plus the sheet's content, not code.
- Which Desmos variant Bluebook ships for Calculus and whether it satisfies all four CED calculator capabilities. The policy page says Bluebook may include graphing, scientific or four-function calculators depending on the exam (https://apstudents.collegeboard.org/exam-policies-guidelines/calculator-policies [verified]). Affects the calculator-part simulator in 05. Settled by: the Bluebook practice app for Calculus, or a College Board statement. Restated 2026-09-24 at P5: still open. The product's panel (`app/web/src/assessment/`) is its own graphing calculator, not Desmos and not any model provider: it plots in a window the student sets, finds zeros, computes a numerical derivative and a numerical definite integral, the four capabilities the CED requires (via research/exam/calculator-policy.md), and shows an explicit radians or degrees indicator, since the 2025 Question 1 stem carries a radian-mode instruction. It appears only on the two calculator-required parts and is absent, not disabled, on the others. If Bluebook's Calculus variant turns out to lack one of the four capabilities, the panel is over-capable, which is the safe direction for the exam's required uses; if it is a full graphing Desmos, the panel is under-featured (no sliders, tables or regressions), which the CED does not require. Settled by: the Bluebook practice app for AP Calculus BC showing which Desmos variant opens on a calculator part and whether it offers zeros, derivative and integral evaluation, or a College Board statement naming the variant.
...
- The score band's placement method (P5, `app/assessment/band.py`) [inferred]. Cut points being unpublished, the composite is placed on a normal curve built from the free-response section's published per-question means and summed standard deviations, read against each published year's score distribution, widened one score point either side, and unioned over years and over every way the provisional points could fall. Its assumptions are shown with the band. Settled by: nothing public settles the cut points; the operator's own mocks beside released-form checkpoints (P7) are the only check on whether the band ever misses.
- The rapid-guessing threshold (P5, `app/assessment/pacing.py`) [inferred]: 10 percent of the student's median untimed time on the archetype once 10 attempts exist, otherwise 4 seconds plus 2 per difficulty factor, and only inside a part's final five minutes. Settled by: the operator's own latency distributions after the first few drills; the flag rate against visibly guessed answers.
